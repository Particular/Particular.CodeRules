namespace NServiceBus.Core.Analyzer.Tests.Helpers
{
    ...
    {
        protected Task Assert(
            string markupCode,
            Action<IEnumerable<(TextSpan, string)>, IEnumerable<(TextSpan, string)>> assertExpectedActual,
            CancellationToken cancellationToken = default) =>
            Assert(Array.Empty<string>(), markupCode, assertExpectedActual, cancellationToken);
        ...
        protected Task Assert(
            ...
            Action<IEnumerable<(TextSpan, string)>, IEnumerable<(TextSpan, string)>> assertExpectedActual,
            CancellationToken cancellationToken = default) =>
            Assert(new[] { expectedDiagnosticId }, markupCode, assertExpectedActual, cancellationToken);
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            assertExpectedActual(expectedSpansAndIds, actualSpansAndIds);
        }
        ...
        protected static Document CreateDocument(string code, params MetadataReference[] extraReferences)
        {
